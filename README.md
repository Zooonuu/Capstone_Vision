...
비전 파트에서 JSON Array 형태로 객체 리스트를 쏠 거야. 여기서 제일 중요한 건, 우리가 이미 내부적으로 가장 위험하고 시급한 물체를 0번 인덱스(parsed_data[0])로 정렬

여러 물체가 들어와도 무조건 배열의 첫 번째 데이터(data[0])만 뽑아서 그 안의 robot_action이 REMOVE면 center_coords로 주행을 시키고, EMERGENCY_STOP이나 ALARM이 뜨면 즉각 주행 취소 및 스피커 모듈
...
train: /home/joo/Desktop/Capstone_Vision/train/images
val: /home/joo/Desktop/Capstone_Vision/valid/images
test: /home/joo/Desktop/Capstone_Vision/test/images

클래스 개수 (동전, 건전지, 그리고 수많은 레고 종류들)
동전 1개, 건전지 1개, 레고 23개 종류 = 총 25개
nc: 25

클래스 이름 목록 (알파벳 순서나 Roboflow 지정 순서로 정렬됨)
주의: 이 이름들이 나중에 risk_db.py의 Key 값과 정확히 일치해야 합니다!
names: [
  'battery',   # 건전지 (Level 3)
  'coin',      # 동전 (Level 2)
  'C1x1x1',    # 여기서부터는 레고 블록들 (Level 2)
  'C2x1x0', 
  'C2x1x1', 
  'C2x2x0', 
  'C2x2x1', 
...
Roboflow 메타데이터 (이 부분은 다운로드 시 자동 생성되므로 건드리지 않아도 됨)
roboflow:
  workspace: gjwns2345-konkuk-ac-kr
  project: final-safety-dataset
  version: 1
  license: CC BY 4.0
  url: https://universe.roboflow.com/gjwns2345-konkuk-ac-kr/final-safety-dataset/dataset/1
